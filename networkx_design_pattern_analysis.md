...
| NumPy 数组 → 图 | `from_numpy_array` | 遍历整个矩阵 | 普通图：遍历整个矩阵，`add_edge` 隐式去重；多重图：上三角过滤 |
| 图 → 稀疏数组 | `to_scipy_sparse_array` | 单一条目 (i, j) | 对称条目 (i, j) 和 (j, i) |
| 稀疏数组 → 图 | `from_scipy_sparse_array` | 遍历所有显式存储条目 | 普通图：遍历所有条目，`add_edge` 隐式去重；多重图：上三角过滤 |
...
| **矩阵转图（普通）** | 遍历整个矩阵 | 遍历整个矩阵，`add_edge` 隐式去重 | 普通无向图 `add_edge` 自动去重 |
| **矩阵转图（多重图）** | 遍历整个矩阵 | 上三角过滤 `u <= v` | 多重图每次 `add_edge` 都创建新边 |
| **稀疏矩阵（图→矩阵）** | 单一条目 `(i, j)` | 对称条目 `(i, j)` 和 `(j, i)` | 存储效率考虑 |
| **稀疏矩阵（矩阵→图，普通）** | 遍历所有显式存储条目 | 遍历所有条目，`add_edge` 隐式去重 | 普通无向图 `add_edge` 自动去重 |
| **稀疏矩阵（矩阵→图，多重图）** | 遍历所有显式存储条目 | 上三角过滤 `u <= v` | 多重图每次 `add_edge` 都创建新边 |
